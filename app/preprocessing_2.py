...
def retrieve_for_subqueries_factory(user_info, retriever, per_query_k: int = 4):
    async def _run(inputs: Dict) -> Dict:
        subqueries: List[str] = inputs.get("subqueries") or [inputs["question"]]
        ...
        for r in retrievers_list:
            if hasattr(r, "search_kwargs"):
                r.search_kwargs["k"] = per_query_k
            if USE_RBAC_PREFILTER:
                apply_rbac_to_retriever(r, user_info)

        async def fetch(q):
            try:
                return await retriever.ainvoke(q)  # async version
            except Exception:
                return retriever.invoke(q)

        lists = await asyncio.gather(*(fetch(q) for q in subqueries))

        # flatten + dedupe
        flat = [d for docs in lists for d in docs]
        seen, unique = set(), []
        for d in flat:
            k = (d.metadata.get("source"),
                 d.metadata.get("page"),
                 d.metadata.get("doc_id"),
                 d.metadata.get("chunk_id"),
                 (d.page_content or "")[:96])
            if k not in seen:
                seen.add(k)
                unique.append(d)
...
def setup_rag_pipeline_with_subqueries(user_info, retriever, generator, memory=None, prompt=None, decompose_generator=None, n_subqueries=QUERY_DECOMPOSITION_N, per_query_k=SUBQUERY_TOP_K):
    ...
    #     }
    # chain = chain | RunnableLambda(retrieve_fn) 

    chain = RunnableParallel({
            "question": RunnablePassthrough(),
            "subqueries": subquery_chain
