    ...
    def _connect(self) -> bool:
        try:
            ch_config = ClickHouseConfig(
                ...
                port=8123,  # Always use HTTP port for clickhouse_connect
            ...
            # Test the connection
            if hasattr(self.ingester, 'test_connection'):
                if not self.ingester.test_connection():
                    raise ConnectionError("Failed to verify ClickHouse connection")
            
            self.logger.info(f"Successfully connected to ClickHouse at {self.config['host']}:8123")
            return True
