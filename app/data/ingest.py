...
from app.utils.vectordb_handler import *
...
# Initialize the VectorDB handlers
milvus_handler = MilvusDBHandler()
milvus_handler.conenct_to_vectorstore(uri=config.MILVUS_URI, collection=config.MILVUS_COLLECTION)

es_handler = ElasticSearchHandler()
es_handler.connect_to_vectorstore(uri=config.ES_URI, index=config.ES_INDEX)

doc_parser = DocumentParser()


def ingest_data(documents) -> None:

    try:            
        # parse all the files 
        documents = doc_parser.parse_documents_from_dict(documents)
        print("type all docs - ", type(documents))
        for d in documents:
            print(d)
        chunks = doc_parser.chunk_documents(documents)
        # print("chunk 0 - ", chunks[0])

        # Load chunks to milvus db
        milvus_vectorstore = milvus_handler.ingest_to_vectorstore(chunks)
        ...
        es_retriever = es_handler.ingest_to_vectorstore(chunks)
...
def ingest_data_with_location(location: str) -> None:
    try:
        ...
        documents = doc_parser.load_documents_in_folder(location)
        for d in documents:
            print(d)
...
def ingest_data_with_db(database: str, username: str, password: str, url: str, port: int, table: str, schema: str, windowHr: int = 72) -> None:
    try:
        milvus_handler = MilvusDBHandler(url, port)
        milvus_handler.conenct_to_vectorstore()

        es_handler = ElasticSearchHandler()
        es_handler.connect_to_vectorstore()
        ...
        # parse all the files 
        documents = doc_parser.parse_documents_with_rbac(file_info)
        for d in documents:
            print(d)
        chunks = doc_parser.chunk_documents(documents)
        # print("chunk 0 - ", chunks[0])
